...
# Purpose: Direct raw filesystem layout regression test.
#
...
with p.open("rb") as f:
    ...
    assert child == 6, child
    assert fatv(6) >= 0x0FFFFFF8, fatv(6)
    assert file_cluster == 7, file_cluster
    assert fatv(7) == 8, fatv(7)
    assert fatv(8) >= 0x0FFFFFF8, fatv(8)
    ...
    for cluster in range(2, 9):
        assert fatv(cluster) != 0, ("internal free cluster", cluster)
