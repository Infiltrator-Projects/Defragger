...
def test_infiltratr_common_integration() -> None:
    ...
    assert (common / "VERSION").read_text().strip() == "1.18.0"
    gitmodules = (ROOT / ".gitmodules").read_text()
    assert "shared/infiltratr-common" in gitmodules
    assert "Infiltrator-Libraries.git" in gitmodules
    cmake = _cmake_source()
    assert "0f458a7145f3a996ff19f11bf9d0893b91ae5cbb" in cmake
    ...
    assert 'COMMON_VERSION="1.18.0"' in local_installer
    assert 'COMMON_COMMIT="0f458a7145f3a996ff19f11bf9d0893b91ae5cbb"' in local_installer
