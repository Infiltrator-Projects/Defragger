...
def test_infiltratr_common_integration() -> None:
    ...
    assert (common / "VERSION").read_text().strip() == "1.19.1"
    gitmodules = (ROOT / ".gitmodules").read_text()
    assert "shared/infiltratr-common" in gitmodules
    assert "Infiltrator-Libraries.git" in gitmodules
    cmake = _cmake_source()
    assert "f7674047a270422862ec8d459b5646a81fe1bfa2" in cmake
    ...
    assert 'COMMON_VERSION="1.19.1"' in local_installer
    assert 'COMMON_COMMIT="f7674047a270422862ec8d459b5646a81fe1bfa2"' in local_installer
