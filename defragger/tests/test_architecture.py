...
def test_infiltratr_common_integration() -> None:
    ...
    assert (common / "VERSION").read_text().strip() == "1.18.1"
    gitmodules = (ROOT / ".gitmodules").read_text()
    assert "shared/infiltratr-common" in gitmodules
    assert "Infiltrator-Libraries.git" in gitmodules
    cmake = _cmake_source()
    assert "dcfa6fee9e9263a0dce5c137054d4adf130c2f25" in cmake
    ...
    assert 'COMMON_VERSION="1.18.1"' in local_installer
    assert 'COMMON_COMMIT="dcfa6fee9e9263a0dce5c137054d4adf130c2f25"' in local_installer
