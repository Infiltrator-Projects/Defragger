...
def handle_run(message: dict[str, Any]) -> None:
    ...
    if not isinstance(argv_raw, list) or not all(isinstance(x, str) for x in argv_raw):
        ...
        return
    with _active_lock:
        if _active_process is not None or (
            _active_thread is not None and _active_thread.is_alive()
        ):
            fail(request_id, "another privileged operation is already active")
            return
    worker = threading.Thread(
        target=run_request,
        args=(request_id, program, list(argv_raw)),
        daemon=False,
    )
    with _active_lock:
...
def stop_active_and_wait() -> None:
    """Request a journal-safe stop and never leave a root writer detached."""

    signalled_pid: int | None = None
    while True:
        with _active_lock:
            process = _active_process
            worker = _active_thread
        if process is not None and process.poll() is None and signalled_pid != process.pid:
            try:
                os.killpg(os.getpgid(process.pid), signal.SIGINT)
                signalled_pid = process.pid
            except ProcessLookupError:
                pass
...
def main() -> int:
    ...
    # EOF means that the GUI disappeared without completing the protocol.
    # The helper remains alive until its writer has reached a safe exit.
    stop_active_and_wait()
    return 0
