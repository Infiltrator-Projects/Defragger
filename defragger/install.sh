...
if [ "$(id -u)" -ne 0 ]; then
    printf '%s\n' 'Defragmenter install.sh must be run as root (for example: sudo ./install.sh).' >&2
    exit 1
fi

command -v cmake >/dev/null 2>&1 || {
    printf '%s\n' 'cmake is required to install Defragmenter.' >&2
    exit 1
}

[ -f "$BUILD/cmake_install.cmake" ] || {
    printf 'No completed CMake build was found at %s. Build Defragmenter first.\n' "$BUILD" >&2
    exit 1
}
