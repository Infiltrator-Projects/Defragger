...
int exfat_clusters_push(ExfatClusterVec *v,uint32_t c){if(v->count==v->capacity){size_t n=v->capacity?v->capacity*2U:16U;v->items=ld_xrealloc(v->items,n*sizeof(*v->items));v->capacity=n;}v->items[v->count++]=c;return 0;}
