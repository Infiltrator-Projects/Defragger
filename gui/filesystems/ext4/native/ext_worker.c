...
static int journal_load(const char *path, ExtJournal *state, char **error) {
    ...
    while (getline(&line, &capacity, file) >= 0) {
        ...
        *equals++ = '\0'; equals[strcspn(equals, "\r\n")] = '\0';
        ...
        else if (strcmp(line, "source_type") == 0) snprintf(state->source_type, sizeof(state->source_type), "%s", equals);
...
static void transaction_cleanup(const char *journal, const ExtJournal *state) {
    if (state != NULL) {
        ...
        if (state->plan != NULL) {
            char *wal = append_suffix(state->plan, "-wal"); char *shm = append_suffix(state->plan, "-shm");
...
static int capacity_preflight(const char *journal_path, const ExtGeometry *geometry, char **error) {
    char *parent = parent_directory(journal_path);
    struct statvfs info;
...
static int build_and_commit(const char *device, const char *operation,
    ...
    uuid_hex(source_geometry.uuid, state.uuid); snprintf(state.source_type, sizeof(state.source_type), "%s", source_geometry.filesystem);
    snprintf(state.operation, sizeof(state.operation), "%s", operation); snprintf(state.phase, sizeof(state.phase), "preflight");
    state.stage = append_suffix(journal_path, ".ext-stage.img"); state.plan = append_suffix(journal_path, ".ext-plan.sqlite");
