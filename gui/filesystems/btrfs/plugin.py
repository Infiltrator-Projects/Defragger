...
from backends.base import (
    ...
    aggregate_ranges, complement_ranges, merge_ranges, overlay_ranges, u16le, u32le,
    u64le,
)
...
def _mark_outside_tail(
    ...
    """Mark the physical device tail beyond Btrfs ``total_bytes`` as outside.

    A Btrfs filesystem can be smaller than its containing partition after a
    deliberate offline boundary reduction.  The allocation aggregator initially
    sees that partition tail as unknown because it is outside the filesystem's
    address space.  Convert only that exact tail to the standard ``outside``
    category so the GUI can distinguish it from unclassified filesystem data.
    """
...
class BtrfsBackend(FilesystemBackend):
    ...
    def map(self, path: str, cells: int) -> dict:
        # Analysis is performed directly against the unmounted device or image.
        # No mount, filesystem ioctl or kernel tree-search interface is used.
        with Reader(path) as reader:
            ...
            filesystem_units = (total_bytes + sectorsize - 1) // sectorsize
            physical_units = (max(total_bytes, reader.size) + sectorsize - 1) // sectorsize
            used_units = [(max(0, start), min(filesystem_units, end)) for start, end in used_units
                          if start < filesystem_units and end > 0]
            free_units = complement_ranges(filesystem_units, used_units)
            ...
            result = _mark_outside_tail(
                result, filesystem_units, physical_units, sectorsize
            )
