...
The desktop, window and About surfaces use one approved 256×256 Defragmenter artwork asset. Packaging installs the same bytes into the desktop icon theme, Mint app-install icon path and the application's private runtime path; the GTK application resolves those installed copies deterministically before consulting the icon theme, so a missing cache entry cannot silently turn the About/window artwork into a placeholder.
