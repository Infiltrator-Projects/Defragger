...
def main() -> int:
    ...
    parser.add_argument("--anchor-mb", type=int, default=2)
    ...
    anchor_size = max(1, args.anchor_mb) * 1024 * 1024
    chunk_size = max(4, args.chunk_kb) * 1024
    print(f"Creating {args.anchors} allocation anchors ({args.anchor_mb} MB each)…", flush=True)
